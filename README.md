...
Easecraft is currently in the **motion-foundation stage**. The token package, `MotionProvider`, reduced-motion hooks, scoped Anime.js lifecycle integration, Next.js component explorer, and Vite consumer fixture are implemented and buildable; no npm package has been published yet.
...
`easecraft-tokens` and the first `easecraft` React APIs are implemented. The Vite fixture consumes `MotionProvider`, `useMotionConfig`, and `useAnime` through the public workspace export as an external application would; all packages remain private and unpublished during foundation work.
