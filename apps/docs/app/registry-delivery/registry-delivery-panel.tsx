...
import { serializeRegistrySources, type RegistryDeliverySources } from "./delivery-types";

interface RegistryDeliveryPanelBaseProps {
  readonly sources: RegistryDeliverySources;
}

type RegistryDeliveryPanelProps = RegistryDeliveryPanelBaseProps &
  (
    | { readonly kind: "component"; readonly slug: ComponentSlug }
    | { readonly kind: "composition"; readonly slug: CompositionSlug }
  );
...
export function RegistryDeliveryPanel(props: RegistryDeliveryPanelProps) {
  const { kind, slug, sources } = props;
  ...
  const titleId = `registry-delivery-${generatedId}`;
  const plan =
    kind === "component"
      ? mode === "package"
        ? getInstallPlan(slug, "package")
        : getInstallPlan(slug, "copy-source")
      : mode === "package"
  ...
  return (
    <section className="composition-delivery" aria-labelledby={titleId}>
      <div className="composition-delivery-heading">
        <div>
          <p className="eyebrow">Delivery contract</p>
          <h2 id={titleId}>Install it</h2>
        </div>
      ...
      <div className="composition-delivery-grid">
        <div className="composition-command-panel">
          ...
          <p>
            {mode === "package"
              ? `The ${kind} imports stable APIs from easecraft.`
              : `The plan includes every transitive Easecraft source file required to compile the ${kind}.`}
          </p>
          <p
            aria-atomic="true"
            aria-live="polite"
          ...
          <div className="composition-plan-heading">
            ...
            <button
              ...
              onClick={() => {
                void copyText(
                  "all",
                  serializeRegistrySources(sourceFiles),
